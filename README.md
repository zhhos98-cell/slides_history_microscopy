...
## Slide-locked bibliography — pass 11 (2026-08-10)

The public bibliography is now at **`slide-locked-pass-11`**: **108 verified entries**, comprising **67 research/collection/conservation studies** and **41 historical primary/object records**, across **10 publication languages**. The machine-readable source of truth is `bibliography/bibliography-manifest.json`; the row data are split across `bibliography-01.csv` through `bibliography-13.csv`. The Pages bibliography reads the manifest dynamically and exports CSV, TSV, JSON, CSL JSON, BibTeX and RIS from the same rows.
...
This module sharpens the bibliography from a list of slide literature into an evidentiary history of the slide economy. Manufacture, standard formats, commercial categories, pricing, distribution, teaching, serial catalogue revision, image publication and later object recovery are recorded as different relations. Instrument-first catalogues from Hartnack, Leitz, Reichert, Plössl and comparable firms, as well as broad anatomical catalogues in which microscopic preparations are incidental, remain outside the bibliography under the same admission rule.

Current search frontier: Austria-Hungary/Bohemia and French/Belgian slide commerce, followed by colonial medical-school and pathology registers in South Asia and southern Africa. The threshold stays unchanged; geographic balance is not produced by admitting sources that merely mention microscopic preparations.
