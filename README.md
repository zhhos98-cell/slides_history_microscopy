...
## Slide-locked bibliography — pass 18 (2026-08-10)

The public bibliography is now at **`slide-locked-pass-18`**: **203 verified entries**, comprising **87 research/collection/conservation studies** and **116 historical primary/object records**, across **19 publication languages**. The machine-readable source of truth is `bibliography/bibliography-manifest.json`; the row data are split across `bibliography-01.csv` through `bibliography-21.csv`. The Pages bibliography reads the manifest dynamically and exports CSV, TSV, JSON, CSL JSON, BibTeX and RIS from the same rows.
...
Pass 18 adds **six verified records**: five primary/object records and one research study. India gains a bounded zoological teaching collection at Hansraj College with **561 permanent slides** and a published category structure. Sri Lanka gains two dedicated museum slide collections at the University of Colombo and University of Sri Jayewardenepura; neither publishes a slide total, so their larger museum-wide specimen counts remain separate.

Singapore gains a physical-to-digital teaching bridge through Krishnan, Rahmanzadeh and Dheen's 2024 NUS-HALO study. The authors selected sections from the Department of Anatomy's existing collection and digitised **160 histology slides across 13 organ systems**. This is admitted as a bounded physical-slide workflow and does not reverse the earlier exclusion of generic virtual-slide platforms.

Southern Africa gains two stronger preparation regimes. UCT Digital Pathology preserves object-level records for numbered members of a long-used student histopathology set, described across the records as about **100 slides**; the set-level scale is retained as approximate because the source wording varies. Cecil Jackson's 1936 Onderstepoort study records a programme to establish a complete collection of microscopic preparations representative of domestic-animal pathology. Its figure of more than **18,000 pathological specimens submitted for histological diagnosis** is explicitly a workload/specimen figure and is not converted into a slide count.

The duplicate audit also matters. The India Office 1851 microscopic-preparations file, Timothy Lewis slide display and J. C. Wagner cabinet were already present from pass 13 and were not re-entered. Generic current teaching labs were held out where they merely listed slides among equipment. Full notes are in `docs/BIBLIOGRAPHY_PASS18_SOUTH_ASIA_SINGAPORE_SOUTHERN_AFRICA_2026-08-10.md`.

Current search frontier: **named medical-school histology/pathology slide registers in Kolkata, Chennai, Mumbai and Sri Lanka; South African historical pathology registers that can connect UCT or Onderstepoort sets to dates, preparators or accession series; residual Singapore/Hong Kong physical glass-slide collections only where a dedicated object/register layer can be recovered; and residual Iberian suppliers where catalogue-to-object links can be closed.** Geological thin sections remain held out pending an explicit scope decision.
