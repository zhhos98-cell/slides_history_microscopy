...
The microscope-slide core has reached **analytical saturation for discovery**. The 10–11 August upgrade sweep produced enough distinct, source-grounded object/document systems to stop broad searching. The default next move for this core is historical synthesis, argument construction and writing.
...
- Resume microscope-core research only from an exact source arrival, a user-supplied object/image, or a newly bounded identifier capable of changing attribution, chronology, object identity, custody or mechanism.

## Current authority

Use `REPOSITORY_STATE.json` as the repository-level authority map. Within the microscope-slide core analysis, use:
...
## Satellite project boundary

`projects/` is a separate namespace. Activity there does not reopen the frozen 307/155 microscope-slide survey and does not change the four microscope-core exact-source requests unless an explicit new survey version says so.

Current satellite routing is documented in `projects/README.md`. The Gusu / Christer von der Burg project continues from its own `README.md`, `BACKUP_STATE.md`, decision log and `latest/` artifacts; project-local `latest` files are not repository-wide authorities.
