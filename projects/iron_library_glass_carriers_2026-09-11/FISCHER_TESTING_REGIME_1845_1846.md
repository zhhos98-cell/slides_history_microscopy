# Fischer 1845–1846 — industrial testing regime before laboratory metallography

Status: active evidence note, 2026-09-11.

Source basis: the supplied machine-readable 1845 and 1846 Johann Conrad Fischer travel texts. These are OCR/research texts, not manually verified diplomatic transcriptions.
...
## 1. Low Moor, 1845 — sample every tapping, number the batch

At Low Moor Ironworks Fischer records quantitative operating data and, crucially, states that a sample was taken from **every tapping** (`von jedem Abstich Muster genommen`). The tapping / pig-iron output was numbered, and Fischer explicitly connects this practice with supplying consumers with iron of a specified required quality.

Working sequence:

`tap / production event -> sample -> numbered batch -> quality distinction -> customer delivery`

This is a traceability and quality-control practice embedded in production, not yet a separate laboratory regime.

Low Moor also gives Fischer a staged portable material series: ore, limestone, coal, coke and iron specimens through the process from pig iron to finished bar iron.
...
`Schaffhausen production -> selected steel bar -> shipment -> Royal Mint trial -> written evaluation -> repeat order / future supply`
...
The operation is therefore both transformation and test:

`known steel -> skilled grinding/polishing/finishing -> expert judgement of resulting quality`
...
Fischer describes a dedicated **testing machine (`Probemaschine`)** beside the locomotive-spring production process. A hardened curved spring is placed in an oiled iron channel. A slowly driven vertical screw presses the spring down until its original bow is flattened against the base. The pressure is then released. The spring is removed and placed between two reference pins; successful return to contact with the pins indicates that it has neither yielded permanently nor broken.

Working sequence:

`finished / hardened spring -> controlled mechanical loading -> forced deformation to specified geometry -> unloading -> recovery against reference points -> quality judgement`

This is a repeatable industrial mechanical test with a defined transformation, reference geometry and pass/fail logic.
...
The factory has observed that two files of equal weight and equal length can behave differently in hardening depending on section geometry: a square file may crack while a flat one survives. Fischer records that the cause has not yet been fully determined.
...
This is not apparatus-based testing. It is a contrasting form of expertise: embodied predictive judgement demonstrated against the behaviour of a live process.
...
At present the evidence supports a structural comparison between industrial testing practices and later laboratory material-testing/metallographic practices. It does **not** yet support:

- direct transmission from Low Moor or a Sheffield spring-testing machine to Wedding/Martens;
- a continuous institutional genealogy;
- the claim that these industrial samples were metallographic specimens;
- the claim that Fischer anticipated microscopic metallography.

Those questions require the Wedding notebooks and post-return Berlin evidence.
